...
3. 从 Claude Code 返回的 stdout 中截取 `!!!CRITICAL!!!` 标记行之后、`!!!WARNING!!!` 标记行之前的所有行（不包含标记行本身），作为严重问题列表。若标记缺失，向用户报告“审核输出格式异常，请人工介入”并停止循环。
...
3. 强制终止（防死循环）：超过 10 轮，或连续 2 轮中至少 1 条严重问题的“文件路径:行号”组合完全重复（以“文件路径:行号”为去重键），或格式校验失败（标记行缺失）。此时向用户报告卡住的问题，请用户介入。
